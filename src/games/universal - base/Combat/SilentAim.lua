...
run(function()
	...
	local deltaIsRK = deltaFramework and deltaFramework.Get and function(player, enemy)
		...
		return isFriendly == false and isSpawnKill == false
	end or nil

	local function canTargetBeKilled(targetPlayer)
		if not targetPlayer then return true end
		if not deltaIsRK then return true end
		local player = deltaEnv and deltaEnv.shared and deltaEnv.shared.Player or nil
	...
	local function getTarget(origin, obj)
		...
		if ent and ent.Player and not canTargetBeKilled(ent.Player) then
			return nil
		end
