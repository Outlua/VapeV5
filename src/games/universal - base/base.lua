warn("RUNNING SCRIPT BASE.LUA IN SRC/GAMES/UNIVERSAL - BASE FOLDER")
...
local function downloadFile(path, func)
    if not isfile(path) then
        ...
        if not suc or res == '404: Not Found' then
            return warn(res, "Failed to download file: "..path .. " from repo with commit ref: "..getCommitRef() .. " at url " .. 'https://raw.githubusercontent.com/Outlua/VapeV5/'..getCommitRef()..'/'..resolveRepoPath(path))n(res, "Failed to download file: "..path .. " from repo with commit ref: "..getCommitRef() .. " at url " .. 'https://raw.githubusercontent.com/Outlua/VapeV5/'..getCommitRef()..'/'..resolveRepoPath(path))
        end
...
local hash = loadstring(downloadFile('src/libraries/hash.lua'), 'hash')()
local prediction = loadstring(downloadFile('src/libraries/prediction.lua'), 'prediction')()
entitylib = loadstring(downloadFile('src/libraries/entity.lua'), 'entitylibrary')()
local whitelist = {
	alreadychecked = {},
	customtags = {},
	tagcallback = {},
	...
	function whitelist:update(first)
		local suc = pcall(function()
			local _, subbed = pcall(function()
				return game:HttpGet('https://github.com/PGN/whitelists')
			end)
			local commit = subbed:find('currentOid')
			commit = commit and subbed:sub(commit + 13, commit + 52) or nil
			commit = commit and #commit == 40 and commit or 'main'
			whitelist.textdata = game:HttpGet('https://raw.githubusercontent.com/PGN/whitelists/'..commit..'/PlayerWhitelist.json', true)
		end)
